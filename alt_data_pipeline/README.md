...
│   ├── storage/               # SQLAlchemy schema, connection, repository, loader
...
docker compose \
    ...
    -f alt_data_pipeline/docker/docker-compose.yml up -d
...
docker compose exec alt-pipeline python -m alt_data_pipeline.run_pipeline \
    --airport KJFK --start 2024-01-01 --end 2024-01-10

# 4. Status
docker compose exec alt-pipeline python -m alt_data_pipeline.run_pipeline --status
```
...
```bash
pytest alt_data_pipeline/tests/unit -v
```
